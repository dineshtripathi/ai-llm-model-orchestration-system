...
st.set_page_config(
    page_title="RAG + Model Orchestration Dashboard",
    page_icon="🔍",
    layout="wide"
)
...
tab1, tab2, tab3, tab4 = st.tabs(["RAG Query", "Document Search", "System Status", "Database Explorer"])
                ...
                if result.get("success", False):
                    ...
                    if use_rag and "retrieved_documents" in result:
                        ...
                        for i, (doc, distance) in enumerate(zip(
                            ...
                            with st.expander(f"Document {i+1} (Similarity: {1-distance:.3f})"):
                                st.write(doc)
                else:
...
with tab3:
    ...
    with col2:
        ...
        st.json({
            "available_models": orchestration_stats["model_pool"]["available_models"],
            "total_requests": orchestration_stats["orchestration"]["total_requests"],
...
with tab4:  # Add this as the 4th tab
    ...
    with col2:
        ...
        selected_source = st.selectbox("Filter by source:", ["All"] + list(source_types.keys()))
        
        if selected_source == "All":
            docs = rag_orchestrator.chroma_manager.collection.get(limit=20, include=["documents", "metadatas"])
        else:
...
if st.button("Crawl Latest Information"):
    ...
    st.success(f"Crawled {result['urls_processed']} sources, created {result['documents_created']} documents")

# Footer
st.markdown("---")
st.caption("RAG + Model Orchestration System - Intelligent Document Retrieval with Multi-Model AI")