...
# Configure the page
st.set_page_config(
    page_title="AI Model Orchestration Dashboard",
    page_icon="🤖",
    layout="wide"
)
...
with st.sidebar:
    ...
    if status:
        ...
        st.error("Unable to connect to orchestration API. Make sure it's running on port 8001.")
...
with col2:
    ...
    if status and status["orchestration"]["total_requests"] > 0:
        success_rate = (status["orchestration"]["successful_routes"] / 
                       status["orchestration"]["total_requests"] * 100)
        st.metric("Success Rate", f"{success_rate:.1f}%")
        st.metric("Avg Routing Time", f"{status['orchestration']['average_routing_time']:.3f}s")
